---
name: agent-delegation-slim-v2
description: 主 Agent 任务分配与子 Agent 精简返回模板（v2）。当需要把独立子任务（搜索、长文档抽取、分类、摘要、格式转换、多模态识别与生成）外包以节省主上下文 token 时使用。提供外包决策规则、子 AI 通道选择（豆包/DeepSeek/普通子 Agent）、转述不设计铁律、delegate_task 工具定义、12 套子 Agent 强制返回 Schema，确保子 Agent 只返回结构化 JSON，不返回原始数据和过程废话。多模态任务必须外包，主 Agent 通过子 Agent 返回的结构化描述实现伪多模态化。v2 相比 v1：路由表优先指向 doubao-v2 / deepseek-web-v2 通道（回退 v1），并纳入 UI 通道的「锚点→候选→验证→降级」发现方法链。变更详情见 CHANGELOG.md；v1 原版保留在同级 agent-delegation-slim 目录。
version: 2.0.0
---
...
|-------|------------------------|------|
| 豆包（本机桌面端） | `doubao-v2`（优先，回退 `doubao`） | 创作/多模态/办公（详见下方路由表） |
| DeepSeek 网页版 | `deepseek-web-v2`（优先，回退 `deepseek-web`） | 深度推理、编程、研究问答 |
...
**环境自适应**：路由只认"当前环境实际有哪些 skill"——环境里多装了 `gpt` 的驱动 skill，路由自动升级到 GPT；少了 `doubao-v2`（回退检查 `doubao`），自动顺延到下一名。**路由表零改动、任何 skill 都不需要写能力声明。**

### 1. 能力优先级快照（2026-08 社区共识，豆包+网络交叉核实；重大模型发布后应重新外派核实）

| 能力 ID | 优先级（AI → 对应 skill 名，按序检查） |
|---------|--------------------------------------|
| ocr_recognize 识图/OCR | Gemini(`gemini`) → GPT(`gpt`) → 豆包(`doubao-v2`，回退 `doubao`) |
| image_generate 图像生成 | Nano Banana(`nano-banana`) → GPT Image(`gpt`) → Midjourney(`midjourney`) → 豆包(`doubao-v2`，回退 `doubao`) |
| video_generate 视频生成 | Seedance 豆包引擎(`doubao-v2`，回退 `doubao`) → Kling(`kling`) → Veo(`gemini`) |
| video_understand 视频理解 | Gemini(`gemini`) → 豆包(`doubao-v2`，回退 `doubao`) → Qwen-VL(`qwen`) |
| writing 写作/文案 | Claude(`claude`) → GPT(`gpt`) → 豆包(`doubao-v2`，回退 `doubao`)（中文语境豆包可直选） |
| coding 编程 | Claude(`claude`) → GPT(`gpt`) → Qwen(`qwen`) → DeepSeek(`deepseek-web-v2`，回退 `deepseek-web`) |
| reasoning 深度推理/研究 | GPT(`gpt`) → Claude(`claude`) → Gemini(`gemini`) → DeepSeek(`deepseek-web-v2`，回退 `deepseek-web`) |
| speech 语音转写/合成 | Gemini(`gemini`) → ElevenLabs(`elevenlabs`) → Whisper(`gpt`) → 豆包(`doubao-v2`，回退 `doubao`) |
| music 音乐生成 | Suno(`suno`) → Udio(`udio`) → Stable Audio(`stable-audio`) → 豆包(`doubao-v2`，回退 `doubao`) |
| document_ppt 文档/PPT | Gamma(`gamma`) → Kimi(`kimi`) → 豆包(`doubao-v2`，回退 `doubao`) |
| translation 翻译 | DeepL(`deepl`) → GPT(`gpt`) → Gemini(`gemini`) → 豆包(`doubao-v2`，回退 `doubao`)（中文互译豆包可直选） |
| web_search 联网搜索 | 主 Agent 自带搜索工具（不经过子 AI，最快最省） |
| agent_task 智能体任务执行 | 豆包工作任务模式(`doubao-v2`，回退 `doubao`) |

> 本机（当前环境）已有 `doubao-v2`、`deepseek-web-v2`、`github-push` 等 skill（v1 同名保留作回退），因此除 coding/reasoning 路由到 DeepSeek 外，其余能力实际都顺延到豆包。UI 自动化通道按 v2 的「先确保窗口就位」+「发现方法链」执行（见下方「UI 通道方法链」一节）。
...
**版本化惯例（v2 起）**：通道 skill 出新版本时用 `-v2` 后缀建新目录（如 `doubao-v2`、`deepseek-web-v2`），v1 原样保留作回退；路由表一律写"优先 v2，回退 v1"——先检查 `xxx-v2` 是否可用，不存在再用 `xxx`。v2 通道自带「窗口就位保障 + 探测工具 + 发现方法链」，对副屏/离屏/改版更抗造。

### 6. UI 通道方法链（v2 新增，对形态一的豆包/DeepSeek 通道适用）

`doubao-v2` / `deepseek-web-v2` 两个 UI 自动化通道已内置同一套健壮性机制，主 Agent 不需要了解内部细节，只需记住两条：
...
> 方法链完整说明与探测工具用法见 `doubao-v2/SKILL.md`、`deepseek-web-v2/SKILL.md` 的「铁律：UI 发现方法链」章节；实测案例见 `doubao-v2/CHANGELOG.md`。
...
### 8. 委派的两种形态（关键区分！写 skill 最容易产生歧义的地方，务必遵守）
...
| 任务类型 | 外包优先级 | 说明 |
|---------|-----------|------|
| 图片识别 / 图像理解 | 必须外包 | **形态一**：直连豆包——`doubao-v2` skill 的 `-Files` 上传图片 + 消息里附 JSON 格式指令，豆包直接返回结构化描述；豆包不可用才降级子 Agent |
| 图像生成 | 必须外包 | **形态一**：加载 `doubao-v2` skill，把用户意图转述给豆包（附格式/尺寸要求），豆包设计生成；成品用 extract / download-asset 取回 |
...
> 创作/生成类任务**不要主 Agent 设计参数、不要用普通子 Agent 凑合**——一律转述给豆包（`doubao-v2` skill），让创作 AI 发挥。
...
2. **直连豆包读取多模态 → 返回结构化文本**：图片/视频/音频通过 `doubao-v2` skill 上传给豆包，消息里附 JSON 格式指令，豆包直接返回结构化描述（JSON）
...
- 主 Agent（形态一）：`doubao-v2` skill `-Files 图片` 发送"请识别此图并按 JSON 返回：{\"objects\":[...],\"scene\":\"...\",\"text_in_image\":\"...\"}"
...
2. 目标执行者是豆包/DeepSeek → **形态一（直连）**：加载对应 skill（`doubao-v2` / `deepseek-web-v2`，不存在则回退 `doubao` / `deepseek-web`），把**任务 + 硬性返回格式写进同一条消息**发出，子 AI 直接按格式返回
...
> 生成类模板（image_generate / video_generate / audio_generate）的定位：**规范"给豆包的转述内容 + 成品回传格式"**（image_url/路径 + 简要描述）。执行通道是 `doubao-v2` skill（转述给豆包生成 → extract / download-asset 取回成品），不是让普通子 Agent 去"生成"。
